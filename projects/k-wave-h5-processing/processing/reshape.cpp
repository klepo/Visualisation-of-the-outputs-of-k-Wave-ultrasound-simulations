...
void Reshape::execute()
{
    ...
    try {
        if ((!getDtsForPcs()->getDatasets(H5Helper::DatasetType::TIME_STEPS_INDEX).empty() || !getDtsForPcs()->getDatasets(H5Helper::DatasetType::BASIC_INDEX).empty()) && getDtsForPcs()->getSensorMaskIndexDataset()) {
...
void Reshape::reshapeMaskTypeDataset(H5Helper::Dataset *dataset, H5Helper::Vector3D globalPos, H5Helper::Vector3D dims, H5Helper::Vector4D chunkDims, bool log)
{
    ...
    for (hsize_t i = 0; i < dataset->getNumberOfBlocks(); i++) {
        ...
        if (!useTmpFlag) {
            for (hssize_t i = 0; i < hssize_t(count.x()); i++) {
                    ...
                    if (datasetType == H5Helper::DatasetType::TIME_STEPS_INDEX) {
                        dstDataset->writeDataset(H5Helper::Vector4D(frame, dstPos.z() - globalPos.z(), dstPos.y() - globalPos.y(), dstPos.x() - globalPos.x()), H5Helper::Vector4D(1, 1, 1, 1), &datasetData[i], false);
                    } else if (datasetType == H5Helper::DatasetType::BASIC_INDEX) {
        ...
        if (i % sensorMaskIndexDataset->getNumberOfBlocks() + 1 == sensorMaskIndexDataset->getNumberOfBlocks()) {
            if (useTmpFlag) {
                if (datasetType == H5Helper::DatasetType::TIME_STEPS_INDEX) {
                    dstDataset->writeDataset(H5Helper::Vector4D(frame, 0, 0, 0), stepSize, tmpData, log);
                } else {
