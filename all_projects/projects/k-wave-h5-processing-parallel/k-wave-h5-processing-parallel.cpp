    ...
    if (dtsForPcs->dtsMaskType.empty()) {
        ...
        try {
            ...
            for (HDF5Helper::File::MapOfDatasets::iterator it = dtsForPcs->dtsMaskType.begin(); it != dtsForPcs->dtsMaskType.end(); ++it) {
                ...
                hsize_t number_of_frames = MAX_NUMBER_OF_FRAMES < dataset->getDims().y() ? MAX_NUMBER_OF_FRAMES : dataset->getDims().y();
                if (MAX_NUMBER_OF_FRAMES == 0)
                    number_of_frames = dataset->getDims().y();
                ...
                if (number_of_frames < (hsize_t) mPISize) {
                    for (int i = 0; i < (int) number_of_frames; i++)
                        hDF5OutputFile->createDatasetF(dataset->getName()  + "/" + std::to_string(i), 3, datasetSize, chunkSize, true);
                } else {
                ...
                for (hsize_t i = 0; i < dtsForPcs->sensorMaskIndexDataset->getNumberOfBlocks() * number_of_frames; i++) {
                    ...
                    if (number_of_frames >= frame + mPIRank + 1) {
                        ...
                        dataset->read3DDataset(HDF5Helper::File::HDF5Vector3D(0, frame + mPIRank, offset.x()), count, datasetData, minVF, maxVF);
                        ...
                        if (number_of_frames < frame + mPISize) {
                            if (number_of_frames > frame)
                                for (hsize_t i = 0; i < number_of_frames - frame; i++)
                                    hDF5OutputFile->createDatasetF(dataset->getName()  + "/" + std::to_string(frame + i), 3, datasetSize, chunkSize, true);
                        } else {
                            for (int i = 0; i < mPISize; i++)
                                hDF5OutputFile->createDatasetF(dataset->getName()  + "/" + std::to_string(frame + i), 3, datasetSize, chunkSize, true);
                        }
...
void resamplingOfDataset(HDF5Helper::File::HDF5Dataset *srcDataset, HDF5Helper::File::HDF5Vector3D nDims, HDF5Helper::File::HDF5Vector3D nDimsDst, HDF5Helper::File::HDF5Dataset *dstDatasetFinal, float &minVG, float &maxVG)
{
    ...
    if (!smallZCountFlag || mPIRank == 0) {
        ...
        dstDatasetFinal->write3DDataset(HDF5Helper::File::HDF5Vector3D(myRound(double (mPIRank) * countZDst), 0, 0), HDF5Helper::File::HDF5Vector3D(countZDstR, nDimsDst.y(), nDimsDst.x()), dataDst2T, true);
...
void resamplingOfDatasetAlltoallv(HDF5Helper::File::HDF5Dataset *srcDataset, HDF5Helper::File::HDF5Vector3D nDims, HDF5Helper::File::HDF5Vector3D nDimsDst, HDF5Helper::File::HDF5Dataset *dstDatasetFinal, float &minVG, float &maxVG)
{
    double t0 = HDF5Helper::getTime();
    ...
    dstDatasetFinal->write3DDataset(HDF5Helper::File::HDF5Vector3D(0, mPIRank * blockDepthG, 0), HDF5Helper::File::HDF5Vector3D(nDimsDst.z(), blockDepth, nDimsDst.x()), dataDst2T, true);
