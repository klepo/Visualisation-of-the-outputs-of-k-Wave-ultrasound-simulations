...
void HDF5File::convertlinearTo3D(hsize_t index, hsize_t &x, hsize_t &y, hsize_t &z)
{
    if (x >= nX) throw std::runtime_error("Wrong offset - too big x offset");
    if (y >= nY) throw std::runtime_error("Wrong offset - too big y offset");
    if (z >= nZ) throw std::runtime_error("Wrong offset - too big z offset");


}

void HDF5File::convert3DToLinear(hsize_t &x, hsize_t &y, hsize_t &z, hsize_t index)
{}
