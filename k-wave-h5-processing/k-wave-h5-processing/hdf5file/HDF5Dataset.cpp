...
HDF5File::HDF5Dataset::HDF5Dataset(H5::DataSet dataset, H5std_string name) : HDF5Object(&dataset)
{
    this->name = name;
...
void HDF5File::HDF5Dataset::readBlock(hsize_t &zO_, hsize_t &yO_, hsize_t &xO_, hsize_t &zC_, hsize_t &yC_, hsize_t &xC_, float *&data, float &minVFTmp, float &maxVFTmp)
{
    if (!blockInitialized) {
        initBlockReading();
    }
    read3DDataset(zO, yO, xO, zC, yC, xC, data, minVFTmp, maxVFTmp);
    zO_ = zO;
    yO_ = yO;
    xO_ = xO;
    zC_ = zC;
...
void HDF5File::HDF5Dataset::readBlock(hsize_t &zO_, hsize_t &yO_, hsize_t &xO_, hsize_t &zC_, hsize_t &yC_, hsize_t &xC_, uint64_t *&data, uint64_t &minVITmp, uint64_t &maxVITmp)
{
    if (!blockInitialized) {
        initBlockReading();
    }
    read3DDataset(zO, yO, xO, zC, yC, xC, data, minVITmp, maxVITmp);
    zO_ = zO;
    yO_ = yO;
    xO_ = xO;
    zC_ = zC;
